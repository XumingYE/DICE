...
class DICE {
	...
	DICE(int DIMENSION, int LINEAR_SIZE, double THRESHOLD, int max_elements):space(DIMENSION) {
        ...
        for (int i = 0; i < 256; ++i) {
            ...
            byte_vectors[i] = arma::normalise(vec); // 将向量存储到byte_vectors中
        }
    ...
    double cosine_similarity(const arma::frowvec& vec1, const arma::frowvec& vec2);
};
...
void DICE::insert(int label) {
    if (alg_hnsw->cur_element_count < alg_hnsw->max_elements_)
        alg_hnsw->addPoint(avg_embedding.memptr(), label);
...
inline double DICE::cosine_similarity(const arma::frowvec& vec1, const arma::frowvec& vec2) {
    double dot_product = arma::dot(vec1, vec2);
    double norm1 = arma::norm(vec1, 2);  // 计算 vec1 的 L2 范数
    double norm2 = arma::norm(vec2, 2);  // 计算 vec2 的 L2 范数
