...
class RevisedNetwork(nn.Module):
    def __init__(self):
        ...
        self.conv_layers = nn.ModuleList([
            ...
            nn.BatchNorm1d(8),
            nn.MaxPool1d(2),
            nn.Conv1d(in_channels=8, out_channels=16, kernel_size=3, stride=1, padding=1, bias=True),
            nn.ReLU(),
            nn.BatchNorm1d(16),
            nn.MaxPool1d(2),
            nn.Conv1d(in_channels=16, out_channels=32, kernel_size=3, stride=1, padding=1, bias=True),
            nn.ReLU(),
            nn.BatchNorm1d(32),
            nn.MaxPool1d(2)
        ])
        ...
        self.fc = nn.Linear(4096, numCluster, bias=False)
...
class HashNetwork(nn.Module):
    def __init__(self, revNet):
        ...
        self.fc_plus = nn.Linear(4096, 128)
        self.fc = nn.Linear(128, numCluster, bias=False)
...
class DeepSketch:
    def __init__(self, DIMENSION, BUFFER_SIZE, THRESHOLD, MAX_ELEMENTS):
        ...
        self.model = HashNetwork(RevisedNetwork())
    ...
    def request(self, chunk:bytes):
        data_tensor = torch.tensor(np.frombuffer(chunk, dtype=np.uint8), dtype=torch.float32).unsqueeze(0)
        _, _, hash_code = self.model(data_tensor)
        self.cur_vector = hash_code.detach().numpy().flatten()
        # print(f'self.cur_vector: {self.cur_vector}')
        # min_dist = float('inf')
        # ret_id = -1
...
def test_my_dice():
    ...
    # 创建 MyDICE 实例
    my_dice = DeepSketch(DIMENSION, BUFFER_SIZE, THRESHOLD, MAX_ELEMENTS)

    # 创建一些随机的字节块
    chunk1 = bytes(np.random.randint(0, 256, 4096))
    chunk2 = bytes(np.random.randint(0, 256, 4096))
    chunk3 = bytes(np.random.randint(0, 256, 4096))
    chunk4 = bytes(np.random.randint(0, 256, 4096))
    chunk5 = bytes(np.random.randint(0, 256, 4096))
