...
class MyFinesse:
    def __init__(self, _SF_NUM, _FEATURE_NUM):
        ...
        self.subchunkIndex = [0] * (_FEATURE_NUM + 1)
    ...
    def request(self, chunk:bytes):
        ...
        for i in range(self.FEATURE_NUM):
            self.feature[i] = 0
            self.subchunkIndex[i + 1] = (len(chunk) * (i + 1)) // self.FEATURE_NUM
        
        for i in range(self.FEATURE_NUM):
            fp = 0
            for j in range(self.subchunkIndex[i], self.subchunkIndex[i] + self.W, 1):
                try:
                    fp *= self.A
                    # fp += int.from_bytes(chunk[j], byteorder='little')
                    fp += chunk[j]
                    fp %= self.MOD
                except:
                    print(self.subchunkIndex)
                    print(len(chunk))
                    print(self.subchunkIndex[i])
            
            for j in range(self.subchunkIndex[i], self.subchunkIndex[i + 1] - self.W, 1):
                if fp > self.feature[i]:
                    self.feature[i] = fp
                fp -= (chunk[j] * self.Apower) % self.MOD
                while fp < 0:
                    fp += self.MOD
                if j != self.subchunkIndex[i + 1] - self.W:
                    fp *= self.A
                    fp += chunk[j + self.W]
                    fp %= self.MOD
        
        groups = [sorted(self.feature[i:i+self.SF_NUM]) for i in range(0, self.FEATURE_NUM, self.SF_NUM)]
        
        # 计算 superfeatures
        for i in range(self.SF_NUM):
            temp = [groups[j][i] for j in range(self.FEATURE_NUM // self.SF_NUM)]
